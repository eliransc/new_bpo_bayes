...
class Simulator:
    def __init__(self, running_time, planner, config_type, reward_function=None, write_to=None, cv=0.25):
        ...
        self.output = [(resource, task) for task in self.task_types[1:] for resource in self.resources if
                       resource in self.resource_pools[task]] + ['Postpone']
        ...
        if sum(self.transitions[current_task.task_type]) <= 1:  # Activity not in parallel
            ...
            for i, p in enumerate(self.transitions[current_task.task_type]):
                if i == len(self.transitions[
                                current_task.task_type]) - 1:  # If the next task is not another task, complete the case
    ...
    def get_state(self):
        ...
        if len(self.available_tasks) > 0:
            task_types_num = [
                sum([1 if task.task_type == el else 0 for task in self.available_tasks]) / len(self.available_tasks) for
                el in self.task_types]
        else:
            task_types_num = [0 for el in self.task_types]
        # + resources_busy_time +
    ...
    def run(self):
        while self.now <= self.running_time:
            ...
            if self.now <= self.running_time:  # To prevent next event time after running time
                if event.event_type == EventType.CASE_ARRIVAL:
                    ...
                    self.events.append(Event(EventType.CASE_ARRIVAL,
                                             self.now + self.sample_interarrival_time()))  # Schedule new arrival
                ...
                if event.event_type == EventType.PLAN_TASKS:
                    ...
                    if self.planner == None:  # DRL algorithm handles processing of assignments (training and inference)
                        ...
                        assignments = self.planner.plan(self.available_tasks, self.available_resources,
                                                        self.resource_pools)
                        for assignment in assignments:
                            self.process_assignment(
                                assignment)  # Reserves the task and resource, schedules TASK_START event
                ...
                if event.event_type == EventType.TASK_COMPLETE:
                    ...
                    if self.config_type == 'complete_parallel':
                        if self.transitions[event.task.task_type][-1] == 1 and sum(
                                self.transitions[event.task.task_type][:-1]) == 0:
                            # Not all parallel tasks are completed, wait for remaining
                            if sum([1 if event.task.parallel_id == task.parallel_id else 0 for task in
                                    self.waiting_parallel_tasks]) != 7 - 1:  # Hard coded nr of parallel tasks
                        ...
                        if event.task.parallel == True:
                            if sum([1 if event.task.parallel_id == task.parallel_id else 0 for task in
                                    self.waiting_parallel_tasks]) != event.task.nr_parallel - 1:
                                self.waiting_parallel_tasks.append(event.task)
        ...
        if self.now > self.running_time:
            ...
            for event in self.events:
                if event.event_type == EventType.TASK_COMPLETE:
                    self.resource_total_busy_time[event.resource] += self.running_time - self.resource_last_start[
                        event.resource]
            ...
            if self.write_to != None:
                utilisation = [busy_time / self.running_time for resource, busy_time in
                               self.resource_total_busy_time.items()]
                resource_str = ''
                for i in range(len(self.resources)):
                    resource_str += f'{utilisation[i]},'
                if self.planner != None:
                    with open(
                            os.path.join(sys.path[0], f'{self.write_to}{self.planner}_results_{self.config_type}.txt'),
                            "a") as file:
                        file.write(
                            f"{len(self.uncompleted_cases)},{resource_str}{self.total_reward},{self.sumx / self.sumw},{np.sqrt(self.sumxx / self.sumw - self.sumx / self.sumw * self.sumx / self.sumw)}\n")
                # else:
            ...
            return len(self.uncompleted_cases), self.total_reward, self.sumx / self.sumw, np.sqrt(
                self.sumxx / self.sumw - self.sumx / self.sumw * self.sumx / self.sumw)
